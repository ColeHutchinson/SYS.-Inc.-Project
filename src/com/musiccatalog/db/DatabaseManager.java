...
public class DatabaseManager {
    ...
    private void seedSampleData(Statement stmt) throws SQLException {
        ...
        boolean seededSongs = rs.getInt(1) == 0;
        if (seededSongs) {
            String insertSongs = """
    ...
    private void ensureEssentialSongs(Statement stmt) throws SQLException {
        stmt.executeUpdate("""
            INSERT INTO songs (title, artist, album, duration_seconds, genre, release_year)
            SELECT 'Bohemian Rhapsody', 'Queen', 'A Night at the Opera', 354, 'Rock', 1975
            WHERE NOT EXISTS (
                SELECT 1 FROM songs WHERE title = 'Bohemian Rhapsody' AND artist = 'Queen'
            )
        """);
        stmt.executeUpdate("""
            INSERT INTO songs (title, artist, album, duration_seconds, genre, release_year)
            SELECT 'Hotel California', 'Eagles', 'Hotel California', 391, 'Rock', 1977
            WHERE NOT EXISTS (
                SELECT 1 FROM songs WHERE title = 'Hotel California' AND artist = 'Eagles'
            )
        """);
        stmt.executeUpdate("""
            INSERT INTO songs (title, artist, album, duration_seconds, genre, release_year)
            SELECT 'Stairway to Heaven', 'Led Zeppelin', 'Led Zeppelin IV', 482, 'Rock', 1971
            WHERE NOT EXISTS (
                SELECT 1 FROM songs WHERE title = 'Stairway to Heaven' AND artist = 'Led Zeppelin'
            )
        """);
        stmt.executeUpdate("""
            INSERT INTO songs (title, artist, album, duration_seconds, genre, release_year)
            SELECT 'Imagine', 'John Lennon', 'Imagine', 187, 'Pop', 1971
            WHERE NOT EXISTS (
                SELECT 1 FROM songs WHERE title = 'Imagine' AND artist = 'John Lennon'
            )
        """);
        stmt.executeUpdate("""
            INSERT INTO songs (title, artist, album, duration_seconds, genre, release_year)
            SELECT 'Billie Jean', 'Michael Jackson', 'Thriller', 294, 'Pop', 1982
            WHERE NOT EXISTS (
                SELECT 1 FROM songs WHERE title = 'Billie Jean' AND artist = 'Michael Jackson'
            )
        """);
    }
